import streamlit as st

One, Two, Three = st.columns([1,2,1])
with Two :
    st.title("  :grey[DeepSign]  ")
    st.write("Bienvenue sur DeepSign, l'application qui rend l'apprentissage de la langue des signes accessible à tous. ")
    st.image("media/Green_theme_1.jpg")
    st.write("Notre mission : ")
    st.write("Rendre la langue des signes accessible au plus grand nombre et favoriser l'inclusion des personnes sourdes et malentendantes dans notre société.")
    st.image("media/Didactik_image_97%.png")
